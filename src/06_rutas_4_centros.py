...
RUTA_RESULTADOS = os.path.join(BASE_DIR, "outputs", "rutas_cuartetos_generados")
...
TOP_CANDIDATOS_POR_ZONA = 4 # Generará un máximo de 4x4x4x4 = 256 escenarios óptimos
...
# 3.3 División Geográfica Inicial (Solo para diversificar la búsqueda de candidatos centrales)
total_demanda = df_nodos_activos['Demanda'].sum()
cuarto_demanda = total_demanda / 4.0
dos_cuartos_demanda = cuarto_demanda * 2.0
tres_cuartos_demanda = cuarto_demanda * 3.0
...
condiciones = [
    df_nodos_activos['Demanda_Acumulada'] <= cuarto_demanda,
    (df_nodos_activos['Demanda_Acumulada'] > cuarto_demanda) & (df_nodos_activos['Demanda_Acumulada'] <= dos_cuartos_demanda),
    (df_nodos_activos['Demanda_Acumulada'] > dos_cuartos_demanda) & (df_nodos_activos['Demanda_Acumulada'] <= tres_cuartos_demanda),
    df_nodos_activos['Demanda_Acumulada'] > tres_cuartos_demanda
]
elecciones = [0, 1, 2, 3]
df_nodos_activos['Zona'] = np.select(condiciones, elecciones, default=3)
mapa_zonas_nodos = df_nodos_activos.set_index('seccion_ine')['Zona'].to_dict()

# 3.4 Clasificar los almacenes físicos en regiones base
...
condiciones_cand = [
    ...
    df_candidatos[eje_corte] > val_corte_3
]
df_candidatos['Zona'] = np.select(condiciones_cand, elecciones, default=3)
...
def generar_rutas_para_centro(candidato, id_zona, id_camion_inicio, id_transp_inicio, mapa_zonas_dinamico):
    ...
    # ¡CRÍTICO! Filtramos usando el mapa balanceado dinámico recalculado para el cuarteto actual
    demanda_pendiente = {
...
# ==========================================
# 6. CRUCE DE CUARTETOS Y CLUSTERIZACIÓN DINÁMICA
# ==========================================
print("\nGenerando simulaciones optimizadas con Clusterización Balanceada (Regret)...")
total_combinaciones = len(top_z0) * len(top_z1) * len(top_z2) * len(top_z3)
contador = 1

# Mapeo rápido de coordenadas de todos los almacenes para evitar consultas lentas
...
for cand0 in top_z0:
    for cand1 in top_z1:
        for cand2 in top_z2:
            for cand3 in top_z3:
                nombres_cuarteto = [cand0['nombre'], cand1['nombre'], cand2['nombre'], cand3['nombre']]
                print(f"🔄 Cuarteto {contador}/{total_combinaciones}: {' 🤝 '.join(nombres_cuarteto)}")
                
                # 6.1 Extracción de coordenadas de los 4 almacenes del cuarteto activo
                coords_centros = {}
                for name in nombres_cuarteto:
                ...
                # 6.2 Cálculo del Regret y de las distancias para cada sección INE activa
                total_demanda_global = df_nodos_activos['Demanda'].sum()
                capacidad_maxima_hub = (total_demanda_global / 4.0) * 1.05  # Límite estricto: Demanda perfecta + 10% tolerancia
                ...
                for idx, row in df_nodos_activos.iterrows():
                    ...
                    distancias_hubs = {}
                    for c_name in nombres_cuarteto:
                    ...
                    regret_valor = ordenados[1][1] - ordenados[0][1] # Penalización por pérdida de asignación óptima
                ...
                # Ordenar de mayor a menor penalización (los clientes críticos en los bordes se asignan primero)
                ...
                carga_hubs = {name: 0 for name in nombres_cuarteto}
                mapa_zonas_dinamico = {}
                centro_to_zona_id = {name: i for i, name in enumerate(nombres_cuarteto)}
                ...
                for idx, row in df_ejecucion_regret.iterrows():
                    ...
                    if not asignado: # Sistema de seguridad por saturación extrema
                        c_emergencia = hubs_por_cercania[0][0]
                        carga_hubs[c_emergencia] += dem_n
                        mapa_zonas_dinamico[nodo] = centro_to_zona_id[c_emergencia]
                
                # 6.4 Lanzamiento del enrutador usando el nuevo mapa concéntrico dinámico
                ...
                rutas_3, _, _ = generar_rutas_para_centro(cand3, id_zona=3, id_camion_inicio=next_camion_3, id_transp_inicio=next_transp_3, mapa_zonas_dinamico=mapa_zonas_dinamico)
                
                rutas_totales_cuarteto = rutas_0 + rutas_1 + rutas_2 + rutas_3
                
                if rutas_totales_cuarteto:
                    df_resultados = pd.DataFrame(rutas_totales_cuarteto)
                    nombre_salida = f"alianza_cuarteto_{cand0['nombre']}_{cand1['nombre']}_{cand2['nombre']}_{cand3['nombre']}.csv"
                    df_resultados.to_csv(os.path.join(RUTA_RESULTS := RUTA_RESULTADOS, nombre_salida), index=False, encoding='utf-8')
                
                contador += 1

print(f"\n🚀 ¡Optimización finalizada! Las simulaciones de cuartetos concéntricos se guardaron en: {os.path.abspath(RUTA_RESULTADOS)}")